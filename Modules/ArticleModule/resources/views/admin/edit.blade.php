...
@section('content')
    <div class="card-header">
        <h4>{{ __('messages.update_article') }}</h4>
    </div>
    <form class="" method="POST" action='{{ route('admin.articles.update', $article->id) }}' enctype="multipart/form-data">
        @csrf
        ...
        <div class="card-body">
            <div class="row mb-3">
                <div class="col-lg-8">
                    ...
                    <input type="text" class="form-control" id="title" name="title"
                        value="{{ old('title', $article->title) }}" placeholder="{{ __('messages.title') }}">
                    @error('title')
                ...
                <div class="col-lg-4">
                    ...
                    <select class="form-select" id="doctor_id" name="doctor_id">
                        ...
                        @foreach ($doctors as $doctor)
                            <option value="{{ $doctor->id }}" @selected(old('doctor_id', $article->doctor_id) == $doctor->id)>
                                {{ $doctor->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3">
                <div class="col-lg-12">
                    <label class="form-label" for="content">{{ __('messages.content') }} <span
                            class="text-danger">*</span></label>
                    <textarea class="form-control tinymce" id="content" name="content" rows="8">{{ old('content', $article->content) }}</textarea>
                    @error('content')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="col-lg-3">
                    @if ($article->pic)
                        <img src="{{ asset('uploads/articles/' . $article->pic) }}" alt="{{ $article->title }}"
                            class="img-fluid mt-4" style="max-width: 90px; max-height: 90px;">
                    @endif
                </div>
                <div class="col-lg-6 d-flex align-items-end">
                    <div class="form-check">
                        ...
                        <input class="form-check-input" type="checkbox" id="status" name="status" value="1"
                            @checked(old('status', $article->status))>
             ...
             <div class="row mb-3">
                <div class="col-lg-6">
                    ...
                    <input type="text" class="form-control" id="slug" name="slug"
                        value="{{ old('slug', $article->seo?->slug) }}" placeholder="{{ __('messages.slug') }}">
                    @error('slug')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="row mb-3">
                <div class="col-lg-12">
                    ...
                    <input type="text" class="form-control" id="meta_title" name="meta_title"
                        value="{{ old('meta_title', $article->seo?->meta_title) }}" placeholder="{{ __('messages.meta_title') }}">
                    @error('meta_title')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="row mb-3">
                <div class="col-lg-12">
                    ...
                    <textarea class="form-control" id="meta_description" name="meta_description" rows="3">{{ old('meta_description', $article->seo?->meta_description) }}</textarea>
            ...
            <div class="row mb-3">
                <div class="col-lg-12">
                    ...
                    <textarea class="form-control" id="meta_tag" name="meta_tag" rows="3">{{ old('meta_tag', $article->seo?->meta_tag) }}</textarea>
                    @error('meta_tag')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="row mb-3">
                <div class="col-lg-6">
                    ...
                    <textarea class="form-control" id="header_script" name="header_script" rows="3">{{ old('header_script', $article->seo?->header_script) }}</textarea>
                    @error('header_script')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-lg-6">
                    ...
                    <textarea class="form-control" id="footer_script" name="footer_script" rows="3">{{ old('footer_script', $article->seo?->footer_script) }}</textarea>
                    @error('footer_script')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
