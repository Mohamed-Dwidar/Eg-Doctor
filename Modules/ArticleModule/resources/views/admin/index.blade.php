...
@section('content')
    ...
    <div class="card-body border-bottom pb-3">
        <form method="GET" action="{{ route('admin.articles') }}" id="filter-form">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-2 d-flex gap-1">
                    ...
                    <a href="{{ route('admin.articles') }}" class="btn btn-secondary btn-sm w-100">
                        <i class="ti ti-x"></i>
                    </a>
                </div>
            </div>
    ...
    <div class="card-body table-border-style">
        <div class="card-header">
            <div class="d-flex justify-content-end gap-2">
                <form action="{{ route('admin.articles.apply-seo') }}" method="POST" id="apply-seo-form">
                    @csrf
                    <button type="button" class="btn btn-outline-primary" id="apply-seo-btn">
                        {{ __('messages.apply_seo_to_all') }}
                    </button>
                </form>
                <a class="btn btn-primary" href="{{ route('admin.articles.add') }}">{{ __('messages.add_new') }}</a>
            </div>
        </div>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th class="align-middle">{{ __('messages.doctor') }}</th>
                        <th class="align-middle">{{ __('messages.views') }}</th>
                ...
                <tbody>
                    @forelse ($articles as $article)
                        <tr>
                            ...
                            <td>
                                @if ($article->doctor)
                                    <a href="{{ route('admin.doctors.edit', $article->doctor->id) }}">
                                        {{ $article->doctor->name }}
                                    </a>
                                @endif
                            </td>
                            <td>{{ $article->views_nu }}</td>
                            <td>
                                @if ($article->status)
                                    <span class="badge bg-success">{{ __('messages.status_published') }}</span>
                                @else
                                    <span class="badge bg-secondary">{{ __('messages.status_draft') }}</span>
                                @endif
                            </td>
                            <td style="width: 5%">
