...
@section('content')
    <div class="card-header">
        <h4>{{ __('messages.update_page') }}</h4>
    </div>
    <form class="" method="POST" action='{{ route('admin.pages.update', $page->id) }}' enctype="multipart/form-data">
        @csrf
        ...
        <div class="card-body">
            <div class="row mb-3">
                <div class="col-lg-6">
                    ...
                    <input type="text" class="form-control" id="title" name="title" readonly
                        value="{{ old('title', $page->title) }}" placeholder="{{ __('messages.title') }}">
                    @error('title')
                ...
                {{-- <div class="col-lg-6">
                    ...
                    <input type="text" class="form-control" id="slug" name="slug"
                        value="{{ old('slug', $page->slug) }}" placeholder="{{ __('messages.slug') }}">
                    @error('slug')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div> --}}
            ...
            {{-- <div class="row mb-3">
                <div class="col-lg-6">
                    ...
                    <select class="form-select" id="parent_id" name="parent_id">
                        ...
                        @foreach ($top_pages as $top_page)
                            <option value="{{ $top_page->id }}" {{ old('parent_id', $page->parent_id) == $top_page->id ? 'selected' : '' }}>
                                {{ $top_page->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-lg-6">
                    ...
                    <select class="form-select" id="is_active" name="is_active">
                        <option value="1" {{ old('is_active', $page->is_active) == 1 ? 'selected' : '' }}>{{ __('messages.yes') }}</option>
                        <option value="0" {{ old('is_active', $page->is_active) == 0 ? 'selected' : '' }}>{{ __('messages.no') }}</option>
                    </select>
                    @error('is_active')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
            ...
            <div class="row mb-3">
                <div class="col-lg-12">
                    ...
                    <textarea class="form-control tinymce" id="content" name="content" rows="5">{{ old('content', $page->content) }}</textarea>
                    @error('content')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            ...
            {{-- <div class="row mb-3">
                <div class="col-lg-12">
                    ...
                    <input type="text" class="form-control" id="meta_title" name="meta_title"
                        value="{{ old('meta_title', $page->meta_title) }}" placeholder="{{ __('messages.meta_title') }}">
                    @error('meta_title')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="row mb-3">
                <div class="col-lg-12">
                    ...
                    <textarea class="form-control" id="meta_keywords" name="meta_keywords" rows="3">{{ old('meta_keywords', $page->meta_keywords) }}</textarea>
                    @error('meta_keywords')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-lg-2">
                    @if ($page->image)
                        <img src="{{ $page->imagePath }}" alt="{{ $page->name }}" class="img-fluid mt-2"
