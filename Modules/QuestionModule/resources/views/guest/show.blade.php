                ...
                <div class="col-lg-7">
                    <div class="egd-doctor-part">
                        ...
                        <div class="egd-ad-slot">
                            ...
                            <p>
                                <!-- Eg-Doctor - Question -  In-article -->
                                <ins class="adsbygoogle" style="display:block; text-align:center;"
                                    data-ad-layout="in-article" data-ad-format="fluid"
                                    data-ad-client="ca-pub-0462453958685277" data-ad-slot="3089359382"></ins>
                                <script>
                                    ...
                                    <div class="egd-answer-card">
                                        <div class="egd-answer-meta">
                                            ...
                                            <span class="text-muted">&middot;
                                                {{ $answer->created?->format('d/m/Y H:i') }}</span>
                                        </div>
                                        <p>{{ $answer->answer }}</p>
                                    </div>
                                @empty
                                    <p class="text-muted mb-0" id="egd-answers-empty">لا توجد ردود حتى الآن، كن أول من يرد.
                                    </p>
                                @endforelse
                            </div>
                            ...
                            <div class="egd-article-detail-card">
                                ...
                                <div id="egd-answer-form-wrapper">
                                    <form method="POST" action="{{ route('questions.answer.store', $question->id) }}"
                                        id="egd-answer-form" novalidate>
                                        @csrf

                                        <div class="egd-form-group">
                                            <label for="writer">الاسم</label>
                                            <input type="text" id="writer" name="writer" value="{{ old('writer') }}"
                                                placeholder="اسمك">
                                        ...
                                        <button type="submit" class="egd-btn" id="egd-answer-submit"><i
                                                class="fas fa-paper-plane"></i> إرسال الرد</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="row g-4">
                                @forelse ($otherQuestions as $otherQuestion)
                                    @php
                                        $egdOtherQuestionUrl = $otherQuestion->seo?->slug
                                            ? url($otherQuestion->seo->slug)
                                            : '#';
                                    @endphp
                                    <div class="col-md-6 col-lg-4">
                                        <a href="{{ $egdOtherQuestionUrl }}" class="egd-consult-card wow fadeInUp"
                    ...
                    <div class="egd-doctor-part">
                                ...
                                @forelse ($randomVideos as $video)
                                    ...
                                    <a href="{{ $egdVideoUrl }}" class="egd-video-item">
                                        <span class="egd-video-thumb">
                                            @if ($egdVideoThumb)
                                                <img src="{{ $egdVideoThumb }}" alt="{{ $video->title }}"
                                                    loading="lazy">
                                            @else
                        ...
                        <div class="egd-ad-slot">
                            ...
                            <p>
                                <!-- Eg-Doctor - Question - Display -->
                                <ins class="adsbygoogle" style="display:block" data-ad-client="ca-pub-0462453958685277"
                                    data-ad-slot="6425794157" data-ad-format="auto"
            ...
            $(function() {
                ...
                $form.on('submit', function(e) {
                    ...
                    if (!answer) {
                        ...
                        egdShowFieldError('answer', 'الرد يتجاوز الحد الأقصى المسموح به (' + ANSWER_MAX_LENGTH +
                            ' حرف).');
                        hasError = true;
                    }
                    ...
                    $.ajax({
                        ...
                        $loading.hide();
                        $wrapper.show();
                        $result.html('<div class="egd-alert-success">' + egdEscape(response.message) +
                            '</div>');
                        $form[0].reset();
                        egdUpdateCharCount();
                        ...
                        if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                            ...
                            $result.html(
                                '<div class="egd-alert-danger">يرجى تصحيح الأخطاء الموضحة أدناه.</div>'
                            );
                        } else {
                            $result.html(
                                '<div class="egd-alert-danger">حدث خطأ غير متوقع، برجاء المحاولة مرة أخرى.</div>'
                            );
                        }
                    });
                });
